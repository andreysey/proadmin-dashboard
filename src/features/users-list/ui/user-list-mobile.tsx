...
export const UserListMobile = ({ table }: UserListMobileProps) => {
  ...
  return (
    <div className="space-y-4 md:hidden">
      {derivedData.map((rowData) => (
        <UserCard
          key={rowData.id}
          rowData={rowData as Row<User> & { optimisticData: OptimisticUser }}
          t={t}
        />
      ))}
    </div>
...
interface UserCardProps {
  rowData: Row<User> & { optimisticData: OptimisticUser }
  t: TFunction
}

const UserCard = ({ rowData, t }: UserCardProps) => {
  const { optimisticData: user, toggleSelected, getIsSelected, getToggleSelectedHandler } = rowData
  ...
  return (
    <Card
      ...
      className={cn(
        ...
        getIsSelected() && 'ring-primary ring-2 ring-offset-2',
        isDeleting &&
